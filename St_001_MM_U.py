...
class OrderbookAlgorizm:
    ...
    def set_principal(self, data):
        ...
        if ret == False:
            ...
            return False
        self.bal_fiat = fiat
        self.bal_coin = self.get_balance(self.ticker)
        # 잔고가 없을 경우
        if self.bal_coin == None:
            self.bal_coin = 0
        currentprice = pyupbit.get_current_price(self.ticker)
    ...
    def set_balance(self, coin):
        ...
        currentprice = pyupbit.get_current_price(self.ticker)
        b = self.get_balance(self.ticker)
        if b == None:
            bal = 0
        else:
            bal = round(b, 8)
        ...
        if startcoin != bal:
            ...
            if startcoin > bal:
                ...
                # 수수료 적용
                price = int(price * (1-0.0005))
                self.systemLog(f"매수할 금액 : {price}")
                if price > 5000:
                    # 매수
                    self.systemLog("시장가 매수")
                    ret = self.buy_market_order(self.ticker, price)
                    order.append(ret['uuid'])
                    # 주문 체결 대기
    ...
    def excute_algorizm(self, data):
        ...
        if self.fiatrate:
            ...
            # 코인 잔고
            self.prin_coin = self.get_balance(str(self.ticker))
            # 잔고가 없을 경우
    ...
    def trade_bid(self, prin, price):
        ...
        # 매수할 갯수 구하기
        # 업비트 거래수수료는 0.05%
        unit = (prin - (prin * 0.0005)) / price
        # 매수 진행 (매수할 티커, 지정가, 매수할 갯수)
        try:
    ...
    def get_balance(self, ticker):
        # 티커 잔고를 구하면 잔고 값만 리턴됨
        return self.u.get_balance(ticker)

    # 주문 취소하는 함수
    def cancel_order(self, uuid):
        self.systemLog("주문 취소")
        try:
            self.u.cancel_order(uuid)
        except:
            pass
            ...
            try:
                ...
                for i in range(0, len(order)):
                    ...
                    if order[i] != 'done':
                        ...
                        ret = self.u.get_order(order[i])
                        # 주문이 체결되었다면
                        if ret['state'] == "done":
                            donenum += 1
                            price = float(ret['price'])
                            amount = float(ret['volume'])
                            volume = price * amount
                            ...
                            self.systemLog(f"{i}번째 주문 체결됨")
                            self.systemLog(f"주문 {donenum}개 체결 완료")
                            order[i] = "done"
                            # 모든 주문이 체결되었는지 확인
                            if donenum == len(order):
                                self.systemLog("모든 주문 체결 완료")
                                waitorder = False
                                return True
                            continue
                        elif ret['state'] == 'cancel':
                            self.systemLog(f"{i}번째 주문 취소됨")
                            print(ret)
                            donenum += 1
                            order[i] = 'done'
                            # 모든 주문이 체결 또는 취소되었는지 확인
                            if donenum == len(order):
    ...
    def sell_market_order(self, ticker, vol):
        self.systemLog("시장가 매도")
        # 매도
        ret = self.u.sell_market_order(ticker, vol)
        return ret
    ...
    def buy_market_order(self, ticker, price):
        self.systemLog("시장가 매수")
        # 매수 진행
        ret = self.u.buy_market_order(ticker, price)
        return ret
    ...
    def buy_limitorder(self, ticker, price, vol):
        ...
        # 매수 진행
        ret = self.u.buy_limit_order(ticker, price, vol)
        return ret
    ...
    def sell_limitorder(self, ticker, price, vol):
        ...
        # 매도 진행
        ret = self.u.sell_limit_order(ticker, price, vol)
        return ret
    ...
    def check_end(self, data):
        ...
        # if (loss != 0 and loss < current) or (profit != 0 and profit > current):
            ...
            if bid:
                ...
                # 코인을 전부 시장가 매도
                vol = self.get_balance(self.ticker)
                # 현재가
    ...
    def get_allbalance(self):
        ...
        # 원금 잔고
        prin_fiat = self.get_balance(str(self.ticker[:3]))
        # 잔고가 없을 경우
        if prin_fiat == None:
            prin_fiat = 0
        # 코인 잔고
        bal_coin = self.get_balance(str(self.ticker[-3:]))
        # 잔고가 없을 경우
        if bal_coin == None:
            bal_coin = 0
        # 현재가
